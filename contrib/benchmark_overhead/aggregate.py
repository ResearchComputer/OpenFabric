...
def cell_metrics(rows: list[dict]) -> dict:
    ...
    totals = [r["client"]["total_ms"] for r in ok if r["client"].get("total_ms") is not None]
    ttfts = [r["client"]["ttft_ms"] for r in ok if r["client"].get("ttft_ms") is not None]
    ...
    return {
        ...
        "total_p50_ms": float(np.percentile(totals_clean, 50)) if totals_clean else None,
        "total_p95_ms": float(np.percentile(totals_clean, 95)) if totals_clean else None,
        "total_p99_ms": float(np.percentile(totals_clean, 99)) if totals_clean else None,
...
def main(run_dir: str) -> None:
    ...
    with per_cell_path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["model", "rps", "path", "rep", "n_total", "n_ok", "error_rate",
                    "total_p50_ms", "total_p95_ms", "total_p99_ms", "total_mean_ms", "total_std_ms",
                    "ttft_p50_ms", "ttft_p95_ms", "ttft_p99_ms"])
        for (model, rps, path, rep), rows in sorted(cells.items()):
            m = cell_metrics(rows)
            w.writerow([model, rps, path, rep, m["n_total"], m["n_ok"], f"{m['error_rate']:.4f}",
                        _fmt(m["total_p50_ms"]), _fmt(m["total_p95_ms"]), _fmt(m["total_p99_ms"]),
                        _fmt(m["total_mean_ms"]), _fmt(m["total_std_ms"]),
                        _fmt(m["ttft_p50_ms"]), _fmt(m["ttft_p95_ms"]), _fmt(m["ttft_p99_ms"])])
    ...
    for (model, rps, path, rep), rows in cells.items():
        m = cell_metrics(rows)
        for k in ("total_p50_ms", "total_p95_ms", "total_p99_ms", "ttft_p50_ms"):
            v = m[k]
            if v is not None:
    ...
    with summary_path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["model", "rps", "path", "reps",
                    "n_total_sum", "n_ok_sum", "error_rate",
                    "p50_median_ms", "p50_std_ms",
                    "p95_median_ms", "p95_std_ms",
                    "p99_median_ms", "p99_std_ms",
                    "ttft_p50_median_ms"])
        for (model, rps, path), d in sorted(agg.items()):
            n_total = int(sum(d["n_total"]))
            n_ok = int(sum(d["n_ok"]))
            err = (n_total - n_ok) / n_total if n_total else 0.0
            w.writerow([model, rps, path, len(d["total_p50_ms"]),
                        n_total, n_ok, f"{err:.4f}",
                        _stat_median(d["total_p50_ms"]), _stat_std(d["total_p50_ms"]),
                        _stat_median(d["total_p95_ms"]), _stat_std(d["total_p95_ms"]),
                        _stat_median(d["total_p99_ms"]), _stat_std(d["total_p99_ms"]),
    ...
    with overhead_path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["model", "rps",
                    "direct_p50_ms", "otela_p50_ms", "overhead_p50_ms",
                    "direct_p95_ms", "otela_p95_ms", "overhead_p95_ms",
                    "direct_p99_ms", "otela_p99_ms", "overhead_p99_ms",
                    "direct_error_rate", "otela_error_rate"])
        ...
        for model in models:
            for rps in rpss:
                ...
                if not d or not o:
                    continue
                d_p50 = float(np.median(d["total_p50_ms"])) if d["total_p50_ms"] else None
                o_p50 = float(np.median(o["total_p50_ms"])) if o["total_p50_ms"] else None
                d_p95 = float(np.median(d["total_p95_ms"])) if d["total_p95_ms"] else None
                o_p95 = float(np.median(o["total_p95_ms"])) if o["total_p95_ms"] else None
                d_p99 = float(np.median(d["total_p99_ms"])) if d["total_p99_ms"] else None
                o_p99 = float(np.median(o["total_p99_ms"])) if o["total_p99_ms"] else None
                d_err = (sum(d["n_total"]) - sum(d["n_ok"])) / sum(d["n_total"]) if sum(d["n_total"]) else 0
                o_err = (sum(o["n_total"]) - sum(o["n_ok"])) / sum(o["n_total"]) if sum(o["n_total"]) else 0
                w.writerow([model, rps,
                            _fmt(d_p50), _fmt(o_p50), _fmt(_sub(o_p50, d_p50)),
                            _fmt(d_p95), _fmt(o_p95), _fmt(_sub(o_p95, d_p95)),
                            _fmt(d_p99), _fmt(o_p99), _fmt(_sub(o_p99, d_p99)),
                            f"{d_err:.4f}", f"{o_err:.4f}"])
