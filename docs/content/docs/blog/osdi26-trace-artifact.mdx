...
```bash
# x86_64
wget https://github.com/eth-easl/OpenTela/releases/latest/download/opentela-amd64 -O otela && chmod +x otela

# arm64
wget https://github.com/eth-easl/OpenTela/releases/latest/download/opentela-arm64 -O otela && chmod +x otela
```
...
```bash
./otela start --mode standalone --public-addr {YOUR_IP_ADDR} --seed 0 --role head
```
...
./otela start \
  ...
  --subprocess "vllm serve Qwen/Qwen3-1.7B --max_model_len 2048 --port 8080" \
  --service.name llm \
  --service.port 8080 \
  --seed 1
```
