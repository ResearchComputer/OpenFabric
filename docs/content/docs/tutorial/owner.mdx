...
## Step 3: Link your wallet to your OpenTela Cloud account

Linking binds the default wallet to your account at [cloud.opentela.ai](https://cloud.opentela.ai) so every peer your nodes run is owned by that account. **One OpenTela account operates a single wallet, and every peer it claims is owned by that wallet** — run the link once per account.

```bash
./otela wallet link --email me@example.com
```

The CLI prompts for your console password, signs a server-issued challenge with the wallet's private key, and submits the signature (`/manage/wallets/challenges` → `/manage/wallets` on `api.opentela.ai`). The private key never leaves the machine. On success:

```
✔ Wallet linked to your OpenTela Cloud account
  Wallet:       5YNmS1R9nNSCDzb5a7mMJ1dwK9uHeAAF4CerBTpfXoA4
  Primary:      true
  Linked at:    2025-06-01T10:35:12Z

Next: claim this node's peers from the console:
  https://cloud.opentela.ai/account
```
...
| Method | How |
| :--- | :--- |
| `--email <addr>` | The CLI prompts for the console password and fetches a fresh JWT itself. |
| `--token <jwt>` | Paste a Neon Auth JWT directly (short-lived; useful for scripted/CI flows). |

| Flag | Config key | Env var | Purpose |
| :--- | :--- | :--- | :--- |
| `--token` | `account.token` | `OF_ACCOUNT_TOKEN` | Neon Auth JWT |
| `--email` | `account.email` | `OF_ACCOUNT_EMAIL` | Console sign-in email |
| `--password` | `account.password` | `OF_ACCOUNT_PASSWORD` | Console password (prompted when omitted on a terminal) |
| `--api-url` | `account.api_url` | `OF_ACCOUNT_API_URL` | Control-plane base URL (default `https://api.opentela.ai`) |
| `--neon-auth-url` | `account.neon_auth_url` | `OF_ACCOUNT_NEON_AUTH_URL` | Neon Auth sign-in URL (defaults to production) |

### If the account already has a wallet

The server rejects a second wallet with `409` (one wallet per account). To switch wallets, remove the current one in the console first — removal only succeeds once it no longer proves an active instance (release or migrate its peers first).

---

## Step 4: Export wallet for third-party use
...
RPC:    https://api.mainnet-beta.solana.com
  ...
  OTELA balance: 10000 OTELA
  OTELA raw:     10000000000000 base units
  OTELA mint:    EsmcTrdLkFqV3mv4CjLF3AmCx132ixfFSYYRWD78cDzR
```

`otela wallet balance` is equivalent. The OTELA line uses `solana.mint` from
`cfg.yaml`, which is set by default to the OpenTela SPL token mint. You can
override it with `--solana.mint <MINT_ADDRESS>`.
...
```bash
./otela balance --solana.rpc https://api.devnet.solana.com
```
...
## Step 8: Get test SOL (devnet airdrop)
...
## Step 9: Service registration with ownership
...
No extra flags are needed for the default wallet — point the node at it and set a value for `wallet.account`:
...
./otela start \
  ...
  --public-addr {YOUR_IP} \
  --seed 0 \
  --wallet.address 5YNmS1R9nNSCDzb5a7mMJ1dwK9uHeAAF4CerBTpfXoA4
```

`--wallet.address` chooses which **managed wallet** the node runs as — useful when the machine holds several (e.g. from earlier imports) or when you want the node to run as exactly the wallet you linked to your cloud account with `otela wallet link`. The address must be in the local store (`otela wallet list`); the node refuses to start with an address it doesn't hold the key for, because it can only attest ownership of a key it controls. Its owner identity, provider ID, and signed attestation all come from that one wallet, so they can never diverge.

The legacy `--wallet.account <pubkey>` override stays supported; if both are given they must agree — prefer `--wallet.address`.
...
## Step 10: Skipping wallet (optional)

If you don't want to use the ownership mechanism (e.g., for quick local testing), you can start a node without a wallet by leaving both `wallet.account` and `wallet.address` empty:
...
| `wallet.account` | `--wallet.account` | `OF_WALLET_ACCOUNT` | Wallet public key override (legacy) |
| `wallet.address` | `--wallet.address` | `OF_WALLET_ADDRESS` | Managed wallet (base58 public key) to run as |
...
| `otela wallet link` | Link the default wallet to your OpenTela Cloud account (one wallet per account) |
| `otela balance` | Show SOL and OTELA balances |
